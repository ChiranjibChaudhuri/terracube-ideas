...
import { loadDggal, type GeoPoint } from './dggal.js';
...
export type UploadJobData = {
  ...
  minLevel?: number;
  maxLevel?: number;
  sourceType?: 'raster' | 'vector' | 'table';
};
...
const toCellRecord = (raw: Record<string, unknown>, fallbackAttrKey?: string): CellRecord | null => {
...
const insertCells = async (datasetId: string, cells: CellRecord[]) => {
  const chunkSize = 500;
  for (let i = 0; i < cells.length; i += chunkSize) {
    ...
    const rows = chunk
      .map((cell, index) => {
        ...
        values.push(
          ...
          cell.value_text,
          cell.value_num,
          cell.value_json ? JSON.stringify(cell.value_json) : null
        );
    ...
    await query(
      `INSERT INTO cell_objects (dataset_id, dggid, tid, attr_key, value_text, value_num, value_json)
       ...
       DO UPDATE SET value_text = EXCLUDED.value_text, value_num = EXCLUDED.value_num, value_json = EXCLUDED.value_json`,
      values
    );
  }
};
...
const getDatasetContext = async (job: UploadJobData): Promise<DatasetContext> => {
  const result = await query<{ name: string; metadata: Record<string, any> }>(
    'SELECT name, metadata FROM datasets WHERE id = $1',
    [job.datasetId]
  );
...
const computeLevelRange = async (cells: CellRecord[]) => {
  if (!cells.length) return { minLevel: undefined, maxLevel: undefined };
...
const geometryCentroid = (geometry: any): GeoPoint | null => {
...
const ingestVectorGeoJSON = async (
  datasetId: string,
  parsed: any,
  attrKey: string,
  level: number
...
export const preprocessUpload = async (job: UploadJobData) => {
  ...
  try {
    ...
    if (extension === 'csv') {
      ...
      if (Array.isArray(parsed)) {
        ...
        records = parsed.cells as Record<string, unknown>[];
      } else {
    ...
    const cells = records
      ...
      .filter((cell): cell is CellRecord => cell !== null);
