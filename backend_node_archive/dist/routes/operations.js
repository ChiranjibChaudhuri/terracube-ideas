...
export const operationRoutes = async (app) => {
    ...
    app.post('/api/ops/spatial', { preValidation: [app.authenticate] }, async (request, reply) => {
        ...
        const datasets = await query('SELECT id, metadata FROM datasets WHERE id = ANY($1)', [[datasetAId, datasetBId]]);
        ...
        if (type === 'intersection') {
            const values = [datasetAId, datasetBId, keyA];
            ...
            values.push(limit);
            const result = await query(`SELECT a.dggid, a.value_num AS value_num, a.value_text AS value_text, b.value_num AS value_num_b, b.value_text AS value_text_b
         FROM cell_objects a
        ...
        values.push(limit);
        const result = await query(`SELECT b.dggid AS dggid, AVG(a.value_num) AS value_num
       FROM cell_objects a
       JOIN cell_objects b ON a.dggid = b.dggid
       WHERE ${clauses.join(' AND ')}
       GROUP BY b.dggid
