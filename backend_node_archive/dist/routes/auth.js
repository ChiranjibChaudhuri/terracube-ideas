...
export const authRoutes = async (app) => {
    app.post('/api/auth/register', async (request, reply) => {
        ...
        const result = await query('INSERT INTO users (email, password_hash, name) VALUES ($1, $2, $3) RETURNING id', [email, passwordHash, name ?? null]);
    ...
    app.get('/api/me', { preValidation: [app.authenticate] }, async (request) => {
        ...
        const result = await query('SELECT id, email, name FROM users WHERE id = $1', [user.sub]);
        if (result.rowCount === 0) {
            return { user: null };
        }
