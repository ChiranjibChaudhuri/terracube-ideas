...
const loadRasterCatalog = async (image, dggrs) => {
    const baseBbox = image.getBoundingBox();
    const extent = buildExtent(baseBbox);
    const images = [image, ...image.getOverviews()];
...
const selectCatalogEntry = (catalog, level) => {
...
const loadRasterData = async (entry, maxPixels, cache) => {
...
const sampleRaster = (raster, point) => {
...
const insertCells = async (datasetId, attrKey, cells) => {
    ...
    for (let i = 0; i < cells.length; i += chunkSize) {
       ...
       DO UPDATE SET value_num = EXCLUDED.value_num`, values);
    }
};
...
export const ingestRasterBuffer = async (options) => {
    ...
    const arrayBuffer = options.buffer.buffer.slice(options.buffer.byteOffset, options.buffer.byteOffset + options.buffer.byteLength);
...
export const bootstrapGebco = async () => {
    ...
    const existing = await query('SELECT id, metadata FROM datasets WHERE name = $1', [
        'GEBCO Bathymetry',
    ]);
    ...
    if (!datasetId) {
        const created = await query('INSERT INTO datasets (name, description, dggs_name, status, metadata) VALUES ($1, $2, $3, $4, $5) RETURNING id', [
            'GEBCO Bathymetry',
    ...
    await query('INSERT INTO attributes (key, description, unit, data_type) VALUES ($1, $2, $3, $4) ON CONFLICT (key) DO NOTHING', ['gebco_depth', 'GEBCO bathymetry depth', 'meters', 'number']);
    let buffer;
